...
export default CardWealthPerson;

const CardComponent: React.FC<Card_Props> = (props: Card_Props) => {
  const { legalData, cardName, t, target } = props;
    ...
    <>
      {legalData ? (
        <KFormCard
          ...
          closedText={ legalData.hasWealth && t(`${cardName}.title`) + `: ` + 
          ( legalData.hasWealth[0] === 1 ? legalData.wealthBase : t(`${cardName}.hasWealthAnswers.${legalData.hasWealth[0] - 1}`) ) }
        >
          <KFormSection>
            <KFormRow>
              <KFormCol width={1}>
                <CardHeading>{t(`${cardName}.title`, { name: state[target || 'myPerson'].firstName } )}</CardHeading>
              </KFormCol>
            </KFormRow>
            <KFormRow>
              <KFormCol width={1}>
