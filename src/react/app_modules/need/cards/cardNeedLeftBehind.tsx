...
class CardNeedLeftBehind {
  constructor() {
    ...
    this.validate = (legalData: any) => {
      ...
      result.total = 1;
      if (!legalData.leftBehindNeed || parseInt(legalData.leftBehindNeed) < 1) result.invalid++;
...
export default CardNeedLeftBehind;
    ...
    <>
      {legalData ? (
        <KFormCard
          ...
          closedText={t(`${cardName}.closedText`, { need: showNum(legalData.leftBehindNeed) })}
          ...
          <KFormSection>
            ...
            <KFormRow>
              <KFormCol width={1 / 2} force={true}>
                <KNumber
                  name={'leftBehindNeed'}
                  label={t(`${cardName}.leftBehindNeed`)}
                  fieldValue={legalData.leftBehindNeed}
              ...
              <KFormCol width={1 / 2} force={true}>
                <GeneralText noMargin>
                  { currentIncome ? 
                  t(`${cardName}.needAdvice`, {recommend: showNum(Math.round(currentIncome * 0.8)) }) :
                  '' }
                </GeneralText>
              </KFormCol>
            </KFormRow>
